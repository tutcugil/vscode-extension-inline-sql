...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const formatCommand = vscode.commands.registerCommand('inlineSql.formatSql', () => {
    formattingProvider.formatSqlAtCursor().catch(err => {
      console.error('Inline SQL: formatSqlAtCursor failed:', err);
    });
  });

  const formatAllCommand = vscode.commands.registerCommand('inlineSql.formatAllSql', () => {
    formattingProvider.formatAllSqlInDocument().catch(err => {
      console.error('Inline SQL: formatAllSqlInDocument failed:', err);
    });
  });
  ...
  if (connConfig) {
    connectionManager.getSchema().catch(err => {
      console.error('Inline SQL: schema fetch failed:', err);
    });
  }
}
